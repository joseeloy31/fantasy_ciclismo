...
def obtener_conexion(ruta_properties):
    ...
    Parámetros:
        ruta_properties (str): Ruta al archivo de configuración .properties que contiene las credenciales y parámetros de la base de datos.

    Salida:
        mysql.connector.connection: Objeto de conexión a la base de datos.

    Lanza:
        ExcepcionConexionBaseDeDatos: Si ocurre algún error al intentar conectarse a la base de datos, como problemas con el archivo de configuración o MySQL.
    """
    ...
    try:
        ...
        raise ExcepcionConexionBaseDeDatos(f"Error de conexión a MySQL: {str(mce)}") from mce

    except Exception as e:
        raise ExcepcionConexionBaseDeDatos(f"Error inesperado al intentar conectarse a la base de datos: {str(e)}") from e